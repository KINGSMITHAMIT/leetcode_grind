...
//     bool check(vector<vector<string>>&board, int r, int c, int n){
//         for(int i = 0; i<=r ; i++)  if(board[i][c] == "Q") return 0;
//         for(int  i = r, j = c; i>=0, j>=0; i--, j--) if(board[i][j]=="Q") return 0;
//         for(int i = r, j =c;i>=0, j<n; i--, j++  ) if(board[i][j]=="Q") return 0;
//         return 1;
//     }
//     void helper(vector<vector<string>>&board, int r, int n){
//         if(r==n) { 
//             ans.push_back(board);   
//             return;
//             }
...
//         string temp(n,'.');
//         vector<vector<string>>board(n,temp);
//         help(board, 0, n);
//         return ans;
//     }
// };
